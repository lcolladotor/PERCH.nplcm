...
#'
#' DN:1.currently only works for both BrS and SS data.
#'
...
#'
#' @return A figure with individual diagnosis plots. The bronze-standard (BrS) measurement pattern is listed
#' at the top of the barplots. The percentage below it is the observed frequency in the actual data.
#' Another 1/0 pattern is shown in red, and represents the silver-standard(SS) measurements.
#' The percentage below it means, within cases having the BrS
#' pattern, the fraction of cases having this SS pattern.
#'
#' @export
nplcm_plot_individual_diagnosis <- function(DIR_NPLCM,npat=16){#BEGIN of function:
            ...
            for (i in 1:length(casepat.high.name)){
              ...
              for (j in 1:length(ind.interest.list.MSS)){
                ...
                text(2*(Jfull/2)-.5,0.9,paste0(round(length(ind.tmp)/Nd,3)*100,"%"),
                     cex=1.5)
                #SS:
                ...
                text(2*(Jfull/2)-.5,.7,paste0(round(length(ind.interest.list.MSS[[j]])/length(ind.tmp),3)*100,"%"),col="red",
                     cex=1.5)
                axis(3,at=2*(1:Jfull)-.5,labels=pathogen_list,
                     las=2,line=-1)
              }
            }
}#END of function
